...
class CancelReceiver : BroadcastReceiver(), KoinComponent {

    private val scope = CoroutineScope(SupervisorJob())
    private val viewModel: CancelViewModel by inject()
            ...
            scope.launch {
                viewModel.apiResponse.collect { apiResponse ->
                    if (apiResponse.status == ApiStatus.SUCCESS) {
                        mBuilder(context, input, "Cancel")?.build()?.let { idTask?.let { it1 -> notificationManager?.notify(it1, it) } }
                    } else {
    ...
    class CancelViewModel(private val apiService: ApiService, private val observer: BaseObserver): BaseViewModel() {

        fun updateTask(idTask: String, description: String) = viewModelScope.launch {
            observer(
                block = {apiService.updateTask(idTask, "cancel", description) },
                toast = false,
